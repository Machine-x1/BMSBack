...
const projectAPIRouter: Router = express.Router();

projectAPIRouter.post("/", upload.array('images', 10), async (req: Request, res: Response) => {
  try {
...
projectAPIRouter.get("/", getProjects);
projectAPIRouter.get("/:slug", getProject);
projectAPIRouter.delete("/:slug", deleteProject);
